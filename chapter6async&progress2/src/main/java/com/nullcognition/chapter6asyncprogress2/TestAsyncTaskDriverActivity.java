...
public class TestAsyncTaskDriverActivity extends Activity implements IReportBack {
   ...
   public void onCreate(Bundle savedInstanceState){
	  ...
	  if((atf = (AsyncTaskFragment)getFragmentManager().findFragmentByTag(ASYNCTASKFRAG)) != null){
   ...
   public boolean onOptionsItemSelected(MenuItem item){
	  ...
	  if(item.getItemId() == R.id.menu_test_async1){
		 if(atf == null){
			atf = AsyncTaskFragment.newInstance("String1", "String2", "String3");
			getFragmentManager().beginTransaction().add(atf, ASYNCTASKFRAG).commit();
		 }
		 return true;
	  }
	  return true;
   ...
   public void allDone(int status){
	  ...
	  getFragmentManager().beginTransaction().remove(atf).commitAllowingStateLoss();
	  atf = null;
   }
}
